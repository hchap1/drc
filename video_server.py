...
_HEADER = struct.Struct('<HBB')    # frame_id (uint16), chunk_index (uint8), total_chunks (uint8)
...
class VideoBroadcaster:
    ...
    def send(self, image) -> None:
        ...
        if total_chunks > 255:
            # frame is too big for the 1-byte chunk-count field -- drop it
            # rather than send something the client can't reassemble
            # (lower JPEG_QUALITY or resolution if you hit this)
            return
